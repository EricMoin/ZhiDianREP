...
import com.example.notes.dialog.EditMenuDialog
...
import com.example.notes.util.MyApplication
...
class RichEditorManager(context: Context, attrs: AttributeSet?): NestedScrollView(context, attrs) {
    ...
    private fun insertNewEditorView(notesInfoData:NotesInfoData,flag:Int){
        ...
        notesEditorDelBtn.setOnClickListener {
            if(visibleCount!=1){
                ...
                Toast.makeText(MyApplication.context,"这已经是最后一条笔记啦！",Toast.LENGTH_SHORT).show()
            }
    ...
    private fun initHyperImageLoader() = HyperManager.getInstance().setImageLoader(NotesImageLoader())
    public fun toEditableStatus(){
        linear.clearFocus()
    ...
    public fun saveData(){
        Toast.makeText(MyApplication.context,"笔记保存中...",Toast.LENGTH_SHORT).show()
        updateTime()
        val notesData = NotesDataManager.currentNotesData
        ...
        Toast.makeText(MyApplication.context,"笔记保存完成",Toast.LENGTH_SHORT).show()
    }
    ...
    fun insertImage(uri: Uri) {
        val imagePath = HyperLibUtils.getFilePathFromUri(MyApplication.context,uri)
